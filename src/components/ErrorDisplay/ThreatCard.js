...
const ErrorDisplay = dynamic(
  () => import("@/components/CodeEditor/CodeDisplay"),
  { ssr: false }
);
...
export default function ThreatCard({ resultList }) {
  ...
  return (
    <div className="overflow-y-auto h-[650px]">
      {resultList.standardize_result.result.map((item, index) => {
        return (
          <div
            key={index}
            className={`mt-4 border border-t-4 rounded-lg flex flex-col items-center justify-start p-6 w-[450px] ${cardBoder(
              item.severity.toLowerCase()
            )} overflow-hidden`}
          >
            <div className="flex items-center justify-start w-full">
              <div
                className={` text-xl bg-${cardColor(
                  item.severity.toLowerCase()
                )} p-1 text-white mr-2`}
              >
            ...
            <div className="w-full items-center justify-start flex mt-6 text-xs">
              ...
              <div className="pl-4 text-[#0000ee]"></div>
            </div>
            {/* <div className="font-normal w-full text-xs mt-2">
              <span className="text-[#727184] font-bold">LOCATION:</span>{" "}
              {LocationFormatter(item.location)}
            </div> */}
            {/* <div className="mt-4">
